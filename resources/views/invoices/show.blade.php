...
@section('content')
    <div class="row">
        <div class="col-md-7">
            <div class="tablet">
                ...
                <div class="tablet__body">
                    <div class="tablet__items">
                        ...
                        @if (Entrust::can('modify-invoice-lines'))
                            @if (!$invoice->sent_at && $invoice->remise_id === null)
                                ...
                                <div class="tablet__item" style="padding: 0;">
                                    <div class="tablet__item__info">
                                        ...
                                        @if ($invoice->remise_id === null && $remises->isNotEmpty())
                                            <form id="invoiceForm" method="post" action="{{ route('remise.add_invoice') }}">
                                                @method('post')
                                                {{ csrf_field() }}
                                                @foreach ($remises as $remise)
                                                    <label>
                                                        <input type="checkbox" name="remise_id"
                                                            value="{{ $remise->id }}" class="single-checkbox">
                                                        {{ $remise->nom }} - {{ $remise->valeur }} %
                                                    </label>
                                                @endforeach
                                                <input type="hidden" value="{{ $invoice->id }}" name="facture_id">
                ...
                color:#337ab7;">
                                                    <i class="icon ion-md-add-circle"></i>
                                                    <span style="font-size:0.7em; font-weight:400;">@lang('Ajouter remise')</span>
                                                </button>
                                            </form>
                                        @endif
                    ...
                    <div class="row">
                        ...
                        @if ($invoice->offer)
                            <div class="col-md-6">
                                ...
                                <p class="invoice-info-subtext">
                                    <button data-offer-external_id={{ $invoice->offer->external_id }} class="btn btn-link"
                                        style="padding: 0px;" id="view-original-offer">@lang('Offer')</button>
                                </p>
                            </div>
                        @endif
                        <hr>
                        ...
                        <div class="col-md-6">
                            @if (Entrust::can('invoice-send'))
                                <button type="button" id="sendInvoice"
                                    class="btn btn-md btn-brand btn-full-width closebtn" value="add_time_modal"
    ...
    @if (!$invoice->sent_at)
        <div class="modal fade" id="SendInvoiceModalConfirm" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-header">
                        ...
                        @if ($apiconnected)
                            ...
                            <select name="invoiceContact"
                                class="small-form-control bootstrap-select contacts-selectpicker" id="user-search-select"
                                data-live-search="true" data-style="btn btn-md dropdown-toggle btn-light"
                                data-container="body">
                            ...
                            <div id="sendMailBox">
                                <label for="sendMail" class="control-label"
                                    id="sendMailCheckboxLabel">@lang('Send mail with invoice to Customer?(Cheked = Yes):')</label>
