...
class Dinero {
  ...
  /* Can also bed used dynamic if you want to use credintals from, controller fx.
  public static function initialize($orgId, $clientId, $clientSecret, $apiKey){
...
public static  function bookInvoice($invoiceGuid, $timestamp) {
   $res = self::getClient()->request('POST', 'https://api.dinero.dk/v1/'. self::$organizationId.'/invoices/'.$invoiceGuid.'/book', [
      'verify' => false,
      'headers' => [
        'Authorization' => 'Bearer ' . self::$accessToken
      ],
  ...
  public static  function sendInvoice($invoiceGuid, $timestamp) {
   $res = self::getClient()->request('POST', 'https://api.dinero.dk/v1/'. self::$organizationId.'/invoices/'.$invoiceGuid.'/email', [
      'verify' => false,
      'headers' => [
        'Authorization' => 'Bearer ' . self::$accessToken
      ],
...
{
       $res = self::getClient()->request('GET', 'https://api.dinero.dk/v1/'. self::$organizationId.'/contacts?field=Name,ContactGuid', [
        'verify' => false, 
        'headers' => [
            'Authorization' => 'Bearer ' . self::$accessToken
        ]
