...
/* For service provider
	$this->app->bind('Pusher', function ($app)
	{
		...
		return new Pusher($keys['public'], $keys['secret'], $keys['appid']);
	}); */
...
/* $options = array(
	...
	$pusher = new Pusher(Config::get('services.pusher.public'), Config::get('services.pusher.secret'), 
		Config::get('services.pusher.appid'),
		 $options);

	$pusher->trigger('demoChannel', 'NewTask', ['test text']);
	*/
 	...
 	public function cvrapistart(Request $vatRequest)
    {
        ...
        if( empty($vat))
        {
            ...
            curl_setopt($ch, CURLOPT_USERAGENT, 'ThasfasfisAtest');
		...
		//$company_name->fill($result['name'])->save();
