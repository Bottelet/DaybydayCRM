...
class PagesController extends Controller {
    ...
    public function dashboard()
    {
    	...
    	$completedTasksToday = Tasks::whereRaw('date(updated_at) = ?', [Carbon::now()->format('Y-m-d')] )->where('status' , 2)->count();
    	$createdTasksToday = Tasks::whereRaw('date(created_at) = ?', [Carbon::now()->format('Y-m-d')] )->count();

        $completedLeadsToday = Leads::whereRaw('date(updated_at) = ?', [Carbon::now()->format('Y-m-d')] )->where('status' , 2)->count();
        $createdLeadsToday = Leads::whereRaw('date(created_at) = ?', [Carbon::now()->format('Y-m-d')] )->count();
