...
class DocumentsTest extends AbstractTestCase
{
    ...
    public function it_creator_of_task_can_view_task_document(): void
    {
        ...
        $this->assertStringStartsWith($document->mime, $response->headers->get('Content-Type'));
    ...
    public function it_assignee_can_view_their_task_document(): void
    {
        ...
        $permissionName = PermissionName::DOCUMENT_VIEW->value;
    ...
    public function it_unrelated_user_cannot_view_document_from_another_users_task(): void
    {
        ...
        /* Act */
        $response = $this->actingAs($this->owner)
            ->from(route('tasks.show', $task->external_id))
            ->get(route('document.view', $document->external_id));
    ...
    public function it_unrelated_user_cannot_view_document_from_another_users_project()
    {
        ...
        /* Act */
        $response = $this->actingAs($this->owner)
            ->from(route('projects.show', $project->external_id))
            ->get(route('document.view', $document->external_id));
    ...
    public function it_unrelated_user_cannot_view_document_from_another_users_client()
    {
        ...
        /* Act */
        $response = $this->actingAs($this->owner)
            ->from(route('clients.show', $otherClient->external_id))
            ->get(route('document.view', $document->external_id));

        /* Assert */
        $response->assertRedirect(route('clients.show', $otherClient->external_id));
        $response->assertSessionHas('flash_message_warning');
    }
    ...
    public function it_unauthorized_user_cannot_upload_file_to_task()
    {
        ...
        /* Act */
        $response = $this->from(route('tasks.show', $this->task->external_id))
            ->post(route('document.task.upload', $this->task->external_id), [
                'files' => [$file],
            ]);
    ...
    public function it_unauthorized_user_cannot_upload_file_to_project()
    {
        ...
        $response = $this->from(route('projects.show', $this->project->external_id))
            ->post(route('document.project.upload', $this->project->external_id), [
                'files' => [$file],
            ]);
    ...
    public function it_upload_to_nonexistent_task_returns_error()
    {
        ...
        $response = $this->from(route('tasks.index'))
            ->post(route('document.task.upload', 'nonexistent-uuid'), [
                'files' => [$file],
            ]);
    ...
    public function it_upload_to_nonexistent_project_returns_error()
    {
        ...
        $response = $this->from(route('projects.index'))
            ->post(route('document.project.upload', 'nonexistent-uuid'), [
                'files' => [$file],
            ]);
