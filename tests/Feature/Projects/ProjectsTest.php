...
class ProjectsTest extends AbstractTestCase
{
    ...
    public function it_can_update_project()
    {
        /* Arrange */
        $project = Project::factory()->create();
        $this->withPermissions(['project-update']);

        /* Act */
        $response = $this->patch(route('projects.update', $project->external_id), [
        ...
        $response->assertSessionHas('flash_message', __('Project successfully updated'));
        $project->refresh();
        $this->assertEquals('Updated Project Title', $project->title);
        // description is run through the HTML purifier (clean()) before saving,
        // which wraps bare text in a <p> tag.
        $this->assertStringContainsString('Updated project description', $project->description);
    }
