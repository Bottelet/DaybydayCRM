...
import { SEED_CLIENT_NAME } from '../../fixtures/users';
...
test.describe('Clients UI', () => {
    ...
    test('can edit a client company name', async ({ page }) => {
        const p = new ClientsPage(page);
    ...
    test('can delete a client created in this test', async ({ page }) => {
        ...
        await p.delete(name);
        await p.assertVisible(name).then(() => { throw new Error('Expected client to be gone'); }).catch(() => {});
    });
});
