...
test.describe('Projects authorization', () => {
  ...
  test('web error returned when project creation fails validation', async ({ page, request }) => {
  ...
  test('json error returned when project creation fails validation', async ({ page, request }) => {
    const response = await request.post(`${PLAYWRIGHT_BASE_URL}/projects`, {
      failOnStatusCode: false,
      headers: {
        Accept: 'application/json',
